...
def normalize_optional_str(value: Optional[str]) -> Optional[str]:
    """
    Normalizes an optional string argument (as received from an MCP tool
    call) by treating null-like placeholder strings a model might send
    (instead of an actual JSON null) as genuinely absent. Reusable anywhere
    an optional string param needs this same defensive handling, not just
    resume_id.
    """
    if value is not None and value.strip().lower() in _NULL_LIKE_STRINGS:
        return None
    return value
...
def resolve_cv(resume_id: Optional[str] = None) -> ResumeCV:
    ...
    """
    resume_id = normalize_optional_str(resume_id)
