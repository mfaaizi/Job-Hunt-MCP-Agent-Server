...
from server.agents.cv_store import normalize_optional_str
...
def search_jobs(
    ...
    country: str = "us",
    language: str = "en",
    remote_only: Union[bool, str, None] = False,
    ...
    Args:
        ...
        remote_only: True for remote/work-from-home jobs only. False (the
            default) includes onsite and hybrid jobs too — to search for
            onsite roles, just leave this False and put "onsite" in the
            query text itself if needed (e.g. "AI engineer onsite in Lahore"),
            rather than trying to pass "onsite" as a value here.
    ...
    country = normalize_optional_str(country) or "us"
    language = normalize_optional_str(language) or "en"
    date_posted = normalize_optional_str(date_posted)

    # remote_only is a soft filter, not essential to the search — if the
    # model sends something we can't interpret as a boolean (e.g. "onsite",
    # a genuine misunderstanding of what this parameter means rather than a
    # null placeholder), don't fail the whole search over it. Fall back to
    # the default instead of raising.
    try:
        remote_only = coerce_optional_bool(remote_only, default=False)
    except ValueError:
        remote_only = False
